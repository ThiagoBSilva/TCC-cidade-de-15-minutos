class CalculoIndiceQueriesConstantes:
    ...
    CALCULAR_INDICE_15MIN_HEXAGONO = '''
        ...
        INSERT INTO t_indice_hexagono (codigo_hexagono, codigo_modalidade_transporte, indice_p1, indice_p2)
        SELECT 
    ...
    CALCULAR_INDICE_15MIN_MUNICIPIO = '''
        INSERT INTO t_indice_municipio (codigo_municipio, codigo_modalidade_transporte, indice_p1, indice_p2)
        SELECT
            malha.codigo_municipio,
            codigo_modalidade_transporte,
            ROUND(SUM(indice_hex.indice_p1) / COUNT(0), 2),
            ROUND(SUM(indice_hex.indice_p2) / COUNT(0), 2)
    ...
    CALCULAR_INDICE_15MIN_UNIDADE_FEDERATIVA = '''
        INSERT INTO t_indice_unidade_federativa (codigo_unidade_federativa, codigo_modalidade_transporte, indice_p1, indice_p2)
        SELECT
            municipio.codigo_unidade_federativa,
            codigo_modalidade_transporte,
            ROUND(SUM(indice_mun.indice_p1) / COUNT(0), 2),
            ROUND(SUM(indice_mun.indice_p2) / COUNT(0), 2)
