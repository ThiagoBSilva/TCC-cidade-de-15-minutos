...
class CalculoIndiceService:
    ...
    def calcular_indice_15min_hexagono(self, conexao_bd: Connection, parametros: dict) -> None:
        try:
            ...
            log.error(msg=f"[{parametros.get('codigo_municipio')}] Houve um erro ao calcular o índice de 15 minutos dos hexágonos para o município. "
    ...
    def calcular_indice_15min_municipio(self, conexao_bd: Connection, parametros: dict) -> None:
        try:
            ...
            log.error(msg=f"[{parametros.get('codigo_municipio')}] Houve um erro ao calcular o índice de 15 minutos para o município. "
                          f"{ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e
    ...
    def calcular_indice_15min_unidade_federativa(self, conexao_bd: Connection, parametros: dict) -> None:
        try:
            ...
            log.error(msg=f"[{parametros.get('codigo_unidade_federativa')}] Houve um erro ao calcular o índice de 15 minutos para a unidade federativa. "
                          f"{ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e