...
class MunicipioService:
    ...
    def buscar_qtde_registros_pendentes_geracao_malha_hexagonal(self, conexao_bd: Connection) -> DataFrame:
        try:
            ...
            log.error(msg=f"Houve um erro ao buscar a quantidade de municípios pendentes de geração da malha hexagonal. {ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e
    ...
    def buscar_qtde_registros_pendentes_extracao_amenidades(self, conexao_bd: Connection) -> DataFrame:
        try:
            ...
            log.error(msg=f"Houve um erro ao buscar a quantidade de municípios pendentes de extração de amenidade. {ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e
    ...
    def buscar_registros_pendentes_geracao_malha_hexagonal(self, conexao_bd: Connection) -> GeoDataFrame:
        try:
            ...
            log.error(msg=f"Houve um erro ao buscar os municípios pendentes de geração da malha hexagonal. {ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e
    ...
    def buscar_registros_pendentes_extracao_amenidades(self, conexao_bd: Connection) -> GeoDataFrame:
        try:
            ...
            log.error(msg=f"Houve um erro ao buscar os municípios pendentes de extração das amenidades. {ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e
    ...
    def buscar_associacoes_origem_destino_por_codigo(self, conexao_bd: Connection, parametros: dict) -> GeoDataFrame:
        try:
            ...
            log.error(msg=f"Houve um erro ao buscar as associações de origens e destinos com os parâmetros {parametros}. {ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e
    ...
    def atualizar_flag_geracao_malha_hexagonal(self, conexao_bd: Connection, parametros: dict) -> None:
        try:
            ...
            log.error(msg=f"Houve um erro ao atualizar a flag de geração da malha hexagonal com os parâmetros {parametros}. {ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e
    ...
    def atualizar_flag_extracao_amenidades(self, conexao_bd: Connection, parametros: dict) -> None:
        try:
            ...
            log.error(msg=f"Houve um erro ao atualizar a flag de extração das amenidades com os parâmetros {parametros}. {ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e
