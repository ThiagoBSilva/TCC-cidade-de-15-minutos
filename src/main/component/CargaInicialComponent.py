...
class CargaInicialComponent:
    ...
    def __tratar_coluna_regiao(self, gdf_unidade_federativa: GeoDataFrame) -> ndarray:
        try:
            ...
            log.error(msg=f"Houve um erro ao tratar a coluna 'regiao' para o GeoDataFrame de unidades federativas. "
                          f"{ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e

    def __montar_coluna_codigo_unidade_federativa(self, gdf_municipio: GeoDataFrame) -> ndarray:
        try:
            ...
            log.error(msg=f"Houve um erro ao montar a coluna 'codigo_unidade_federativa' para o GeoDataFrame dos municípios. "
                          f"{ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e
    ...
    def montar_carga_inicial(self) -> tuple[GeoDataFrame, GeoDataFrame]:
        try:
            ...
            gdf_unidade_federativa = DataFrameUtil.renomear_colunas_dataframe(
                df=gdf_unidade_federativa, mapeamento=MapeamentosConstantes.COLUNAS_UNIDADE_FEDERATIVA_SHAPEFILE)
            ...
            gdf_municipio["codigo_unidade_federativa"] = self.__montar_coluna_codigo_unidade_federativa(gdf_municipio)

            log.info(msg="Os dados para a carga inicial foram montados com sucesso.")

            return gdf_unidade_federativa, gdf_municipio
        
        except Exception as e:
            log.error(msg=f"Houve um erro ao montar a carga inicial. "
                          f"{ExceptionUtil.montar_erro_exception_padrao(e)}")
            raise e